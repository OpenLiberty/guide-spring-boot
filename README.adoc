...
:page-seo-title: Containerizing, packaging and running a Spring Boot application with Open Liberty.
:page-seo-description: A tutorial on how to containerize, package and run a Spring Boot application without modification in a Docker container with Open Liberty, on a local Open Liberty server, and how to package it embedded with an Open Liberty runtime.
:guide-author: Open Liberty
= Containerizing, packaging and running a Spring Boot application

Learn how to containerize, package and run a Spring Boot application on a Liberty server without modification.

== What you'll learn
The starting point of this guide will be the finished application from Spring's https://spring.io/guides/gs/spring-boot/[Building an Application with Spring Boot^] guide. If you are not familiar with Spring Boot, go through that guide first. Note that Java 8 is required to run this project.

You will learn how to deploy a Spring Boot application on a Liberty server in Docker without modification, by using the thin command of Liberty's Spring Boot utility `springBootUtility`, which stores the dependent library JARs of the application in a target library cache and packages the remaining application artifacts into a thin application JAR.
...
== Building and running the application in Docker container
...
The first stage copies over the Spring Boot application [hotspot=copyJar]`guide-spring-boot-0.1.0.jar` to a temporary directory [hotspot=7 file=0]`/staging`, 
...
The next stage starts from the [hotspot=OLimage2 file=0]`Open Liberty Docker image`, then copies over
...
Your image `springboot` should appear in the list of all Docker images as such:
[role='no_copy']
```
...
You can watch the application start by monitoring the logs (press `CTRL+C` to exit):
[role='command']
```
docker logs -f springBootContainer
```

When the application has started, point your browser to the http://localhost:9080/hello[^] URL to access the application.
...
Run the `package` goal if you didn't build the Spring Boot application.
[role='command']
```
./mvnw package
```

Next, run the `liberty:run` goal that create the Liberty, install required features, and deploy the Spring Boot application to the Liberty, and then start the application.
[role='command']
```
./mvnw liberty:run
```
...
After you are done checking out the application, stop the Open Liberty server by pressing `CTRL+C` in the shell session where you ran the server. Alternatively, you can run the `liberty:stop` goal from the `start` directory in another shell session:
[role='command']
```
./mvnw liberty:stop
```
...
You can update the `pom.xml` to bind more Liberty Maven goals to the package phase, so that the Maven `package` goal can build the Spring Boot application embedded with Open Liberty.
...
---- 
[role="edit_command_text"]
Add the [hotspot=include file=0]`<include/>` and [hotspot=packageFile file=0]`<packageFile/>` configuration tags, and the [hotspot=packageGoals file=0]`<executions/>` tag to the `pom.xml`. 
...
The [hotspot=packageGoals file=0]`<executions/>` tag specifies the required Liberty Maven goals to generate the application embedded with Open Liberty. 
...
Run the repackaged Spring Boot application with Liberty. This jar was defined previously in the [hotspot file=0]`pom.xml`.
