...
The [hotspot=thin file=0]`<exec />` tag in  the [hotspot file=0]`pom.xml` executes the `springBootUtility` command to create thin Spring Boot applications and library caches that can be deployed on Liberty.
The `--targetThinAppPath` option specifies the path used to save the thin application file which we will rename in the `server.xml`.
