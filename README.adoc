...
== What you'll learn
The starting point of this guide is the finished application from Spring's https://spring.io/guides/gs/spring-boot/[Building an Application with Spring Boot^] guide.
...
Then, the Dockerfile copies the Spring Boot dependent library JAR files that are at the [hotspot=libcache file=0]`lib.index.cache` directory and
the [hotspot=thinjar file=0]`thin-guide-spring-boot-0.1.0.jar` file.
