...
ifndef::cloud-hosted[]
=== Faster startup with Open Liberty InstantOn
...
[role='command']
```
docker commit springboot-checkpoint-container springboot-instanton
...
You can run the following command to see the logs:
[role='command']
```
docker logs springBootContainer
```
