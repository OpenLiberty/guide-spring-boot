...
configuration file, and copies the library cache [hotspot=libcache file=0]`lib.index.cache` and 
thinned application [hotspot=thinjar file=0]`thin-gs-spring-boot-0.1.0.jar` which were generated in the first stage.
...
The [hotspot=servlet file=1]`servlet` and [hotspot=springboot file=1]`springBoot` features are required for the Liberty server to run the Spring Boot application.  The application port is specified as [hotspot=httport file=1]`9080`.
