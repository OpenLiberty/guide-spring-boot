...
:page-majorupdateddate: 2024-12-15
...
[role='command']
```
docker run --rm -d --name springBootContainer -p 9080:9080 -p 9443:9443 springboot
```
...
After you are finished checking out the application, stop and remove your container by running the following command:
[role='command']
```
...
[role='no_copy']
----
CWWKF0011I: The defaultServer server is ready to run a smarter planet. The defaultServer server started in 7.121 seconds.
----
...
ifndef::cloud-hosted[]
=== Faster startup with Open Liberty InstantOn - Optional

In this section, you’ll learn how to build an InstantOn image that provides fast startup times. Your application can start in milliseconds, without compromising on throughput, memory, development-production parity, or Java language features. To learn more about the Liberty InstantOn feature, visit the https://openliberty.io/docs/latest/instanton.html[Faster startup for containerized applications with Open Liberty InstantOn^] document.

If you are working on a Linux system with kernel version 5.9 or greater, you can continue on this section, otherwise, go to the https://openliberty.io/guides/spring-boot.html#running-the-application-on-open-liberty[Running the application on Open Liberty] section.
...
You built the `springboot` docker container image in the previous section. Run the following command to take a the checkpoint of the application process with the `afterAppStart` option:
...
The `springBootCheckpointContainer` container from the previous step contains the data from the InstantOn checkpoint process. Take this checkpoint process data and commit it to an application container image layer by running the following commit commands:
...
You can run the following command to see the container logs:
[role='command']
```
docker logs springBootContainer
```
...
[role='no_copy']
----
CWWKF0011I: The defaultServer server is ready to run a smarter planet. The defaultServer server started in 0.323 seconds.
----
