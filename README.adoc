...
The [hotspot=installAppPackages file=0]`<installAppPackages />` tag in the [hotspot file=0]`pom.xml` typically takes in the following parameters: `dependencies`, `project`, `all` or in this case `spring-boot-application`. The default value being `dependencies`. 
However, in order to install the Spring Boot application to the downloaded Open Liberty, the value should be set as `spring-boot-application`. This allows maven to package, thin, and copy the Spring Boot application `gs-spring-boot-0.1.0.jar` to the Liberty runtime application and shared library directories.
...
Since the  application was thinned, the `<springbootApplication />` location is set to [hotspot=location file=1]`thin-gs-spring-boot-0.1.0.jar`. The application port is also specified as [hotspot=10 file=1]`9080`.
...
The first stage copies over the Spring Boot application [hotspot=copyJar]`gs-spring-boot-0.1.0.jar` to a temporary directory [hotspot=4]`/staging`, 
[hotspot=0]`hotspot 0`, [hotspot=1]`hotspot 1`, [hotspot=2]`hotspot 2`, [hotspot=3]`hotspot 3`, [hotspot=4]`hotspot 4`, [hotspot=5]`hotspot 5`, [hotspot=6]`hotspot 6`, [hotspot=15]`hotspot 15`
...
The next stage downloads the [hotspot=OLimage2]`Open Liberty image`, copies the `server` 
