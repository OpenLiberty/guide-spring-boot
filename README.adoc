...
The [hotspot=servlet file=1]`servlet` and [hotspot=springboot file=1]`springBoot` features are required for the Liberty server to run the Spring Boot application. The application port is specified as [hotspot=httpport file=1]`9080` and the application is configured as a [hotspot=springBootApplication file=1]`springBootApplication` element. For more information about the `springBootApplication` element, see the https://www.openliberty.io/docs/latest/reference/config/springBootApplication.html[Server configuration/springBootApplication documentation^].
