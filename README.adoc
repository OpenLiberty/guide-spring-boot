...
:page-seo-description: A tutorial with examples on how to containerize, package, and run a Spring Boot application in a Docker container on an Open Liberty instance.
...
Learn how to containerize, package, and run a Spring Boot application on an Open Liberty instance without modification.
...
You will learn how to use the `springBootUtility` command to deploy a Spring Boot application in Docker on an Open Liberty instance without modification. This command stores the dependent library JAR files of the application to the target library cache, and packages the remaining application artifacts into a thin application JAR file.

You will also learn how to run the Spring Boot application locally with an Open Liberty instance, and how to package it so that it is embedded with an Open Liberty instance.
...
To run the Spring Boot application, the Open Liberty instance needs to be correctly configured. By default, the `liberty-maven-plugin` picks up the server configuration file from the `src/main/liberty/config` directory.
