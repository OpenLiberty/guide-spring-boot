...
== Building and runinng the Spring Boot app
...
Notice that the console output will display that the application is running in Tomcat on port 8080. Now access the application at the following URL: http://localhost:8080[^].
...
server.xml
[source, XML, linenums, role='code_column hide_tags=sbThin'] 
---- 
...
Point your browser to the http://localhost:8080[^] URL to access the application.
...
The jar generated for a spring boot application contains various supporting artifacts that can make it very large. If you are updating your application frequently in a cloud environment, pushing a large application can be resource intensive. 
You can update the `pom.xml` and the `server.xml` to to separate the application code from the dependency jars, making it more efficient to deploy new versions of the application.
...
---- 
[role="edit_command_text"]
Add the [hotspot=sbThin file=1]`<springBootApplication />` tag.
...
== Deploying the application to Open Liberty (to be removed)
...
=== Quick deploying the application to dropins (to be removed)
...
Now access the application at the following URL http://localhost:9080[^].
...
=== Installing your application in the apps directory (to be removed)
...
== Thinning the application with the springBootUtility thin task (to be removed)
...
As before, you can ensure that the application is running by going to the following URL http://localhost:9080[^].
...
== Deploying the application to Docker (to be removed)
