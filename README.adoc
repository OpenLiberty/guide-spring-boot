...
== What you'll learn
You will learn how to run a Spring Boot application in a Open Liberty Docker container, how to run it by a local Liberty server, and how to repackage it with a Liberty server.
...
You will first learn how to take the finished application from that guide and execute it in a jar. Then, using that jar you will learn how to containerize and deploy the application using an Open Liberty image.
Next, you will learn about running the Spring Boot application in Open Liberty locally and also how to package the  application embedded with Open Liberty into a jar. 
...
Notice that the console output displays a message that the application is running in Tomcat on port 8080. Now access the application at the following URL: http://localhost:8080/hello[^].
...
The first stage copies over the Spring Boot application [hotspot=copyJar]`gs-spring-boot-0.1.0.jar` to a temporary directory [hotspot=7]`/staging`, 
...
The next stage downloads the [hotspot=OLimage2]`Open Liberty image`, copies the [hotspot=20]`server` 
configuration file, and copies the library cache [hotspot=libcache]`lib.index.cache` and 
thinned application [hotspot=thinjar]`thin-gs-spring-boot-0.1.0.jar` which were generated in the first stage.
...
The [hotspot=servlet file=2]`servlet` and [hotspot=springboot file=2]`springBoot` features are required for the Liberty server to run the Spring Boot application.
...
Since the  application was thinned, the `<springbootApplication />` location is set to [hotspot=location file=1]`thin-gs-spring-boot-0.1.0.jar`. Adding this location tag in the `server.xml` is optional since the thinned application was packaged in the shared library directories earlier.  The application port is also specified as [hotspot=14 file=1]`9080`.
