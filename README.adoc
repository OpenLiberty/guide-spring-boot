...
The [hotspot=thin file=0]`<exec />` tag in  the [hotspot file=0]`pom.xml` executes the [hotspot=36 file=0]`springBootUtility` command to create thin Spring Boot applications and library caches that can be deployed on Liberty.
...
Next, runt he repacked spring boot application with Liberty.
