...
Add the [hotspot=libertyMavenPlugin]`liberty-maven-plugin` to the [hotspot]`pom.xml`.
...
However, in order to run the Spring Boot application the value will be set as `spring-boot-application`. This allows maven to package and copy the `spring-boot-project` to Liberty's runtime's application directory. This specification also separates the application code from the dependency jars, creating a more efficient "thinned" version of the application.

The [hotspot=configFile file=0]`<configFile />` tag specifies the location of the configuration file, in this case it is the `server.xml` which you will create.
...
The `<include />` tag of this execution is specified to `minify`, `runnable`. The `runnable` value allows the application to be generated as a `jar` file. 

The `<packageFile />` tag specifies that the application will be generated as `GSSpringBootApp.jar`. 
