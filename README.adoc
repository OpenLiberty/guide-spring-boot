...
"The first stage copies over the Spring Boot application gs-spring-boot-0.1.0.jar to a temporary directory /staging, and then use the Open Liberty springBootUtility command to thin the application."

This Dockerfile is written in two main stages. The first stage copies over the Spring Boot application [hotspot=copyJar]`gs-spring-boot-0.1.0.jar` to a temporary directory `/staging`.



that was generated into the [hotspot=OLimage1]`Open Liberty image` and thins the application.
