...
ifndef::cloud-hosted[]
== Faster startup the container with Open Liberty InstantOn - Optional

In this section, you’ll learn how to build an InstantOn image that provides fast startup times. Your application can start in milliseconds, without compromising on throughput, memory, development-production parity, or Java language features. To learn more about the Liberty InstantOn feature, see the https://openliberty.io/docs/latest/instanton.html[Faster startup for containerized applications with Open Liberty InstantOn^] documentation.

If you are working on a Linux system with kernel version 5.9 or greater, you can continue on this section, otherwise, go to the <<running-the-application-on-open-liberty>> section. You can run the following command to check out the kernel version of your system:
