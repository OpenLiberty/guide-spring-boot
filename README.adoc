...
However, in order to install the Spring Boot application to the downloaded Open Liberty, the value should be set as [hotspot=installAppPackages file=0]`spring-boot-application`. This allows maven to package, thin, and copy the Spring Boot application `gs-spring-boot-0.1.0.jar` to the Liberty runtime application and shared library directories.
...
The next stage downloads the [hotspot=OLimage2]`Open Liberty image`, copies the [hotspot=OLimage2]`server` 
